...
<?php
        ...
        if ($_POST["nuevo"]=="guardar")
				...
				$mov=new acciones();
				$ms= $mov->guardar("limitegeo","idlimitegeo,nombre","'$a','$b'");
               }
            }
        ?>
...
<html lang="es">
    <head>
        <title>Limite Geograf&iacute;ca</title>
...
<div id='todo'>
	...
	<section>
		...
		<nav id="menu2">
                    <table id="otromenu">
			<td><input type=image src="../libreria/images/forms.png" id="nuevo" name="nuevo" value="modificar" title="Modificar" onclick="location.href='mlimite.php';"></td>
		...
		<fieldset>
                <legend>
                Limite Geograf&iacute;ca
                </legend>
                 <table id="tabla">
				 
				  <tr><td><label>Codigo:</label></td>
				  <td><input type="text" name="codigo" id="codigo" value="<?php echo newid('limitegeo','idlimitegeo'); ?>" readonly></td></tr>
                  
				  <tr><td><label>Limite Geograf&iacute;ca:</label></td>
				  <td><input type="text" name="nombre" value="" required="required" autofocus title='Digite Direcci&oacute;n'></td></tr>		 
							</table> 
						</fieldset>			 
					</form>		
					...
					<?php if ($ms!='') echo " <script>alert('$ms');</script>"; ?>
						...
						<form action="limitegeo.php" method="get">
						<table>
			...
			{  //para buscar
				...
				$b=new consultas();
				echo $b->buscarvalor("limitegeo","nombre","idlimitegeo='$ii'");
			}
			...
			echo $con->consultar("limitegeo","idlimitegeo as'Codigo',nombre as 'Limite Geograf&iacute;ca'","idlimitegeo order by idlimitegeo desc limit 5");
