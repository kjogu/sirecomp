...
if (isset($_GET['id']))
	{
		...
		$id=$_GET['id'];
		$b=new consultas();
		$modno=$b->buscarvalor("tipoayuda","nombreinsti","idtipoayuda='$id'");
		$modidco=$b->buscarvalor("tipoayuda","idcomunidad","idtipoayuda='$id'");
 	...
 	if (isset($_GET['idb']))
	{
		...
		$id=$_GET['idb'];
		$b=new acciones();
		$mod1=$b->borrar("tipoayuda","idtipoayuda='$id'");
...
?>	
		...
		<form class="comunidad" method="POST" action="modificarco.php" >
			...
			<td><input type="text" name="nombre" value="<?php echo $modno; ?>" required="required" autofocus title='Digite Nombre Insituci&oacute;n' placeholder="Nombre Instituci&oacute;n"></td></tr>
		...
		<?php }
			if (isset($_POST["codigo"]))
			{
				...
				$mov=new acciones();
				echo $mov->Modificar("tipoayuda","nombreinsti='$b',idcomunidad='$c'","idtipoayuda='$a'");}	?>
