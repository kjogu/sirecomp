...
if (isset($_GET['id']))
	{
		...
		$id=$_GET['id'];
		$b=new consultas();
		$modno=$b->buscarvalor("riesgo","nombreri","idriesgo='$id'");
		$modfei=$b->buscarvalor("riesgo","fechain","idriesgo='$id'");
		$modfef=$b->buscarvalor("riesgo","fechafi","idriesgo='$id'");
		$modidco=$b->buscarvalor("riesgo","idcomunidad","idriesgo='$id'");
		$modidob=$b->buscarvalor("riesgo","idobras","idriesgo='$id'");
 	...
 	if (isset($_GET['idb']))
	{
		...
		$id=$_GET['idb'];
		$b=new acciones();
		$mod1=$b->borrar("riesgo","idriesgo='$id'");
		...
		<div id="menu">
            <ul>
				...
				<li><a href='../index.php'>Cerrar Sesi&oacute;n</a></li>
						...
						<form action="modificarri.php" method="get">
						<table>
...
?>	
        <div id='infoprin'>  
		<form class="comunidad" method="POST" action="modificarri.php" >
			<fieldset>
                <legend>
                Datos del Riesgo y Amenazas
                </legend>
			...
			<td><input type="text" name="nombre" value="<?php echo $modno; ?>" required="required" autofocus placeholder='Nombre Riesgo' title='Nombre Riesgo'></td></tr>
            
			<tr><td><label>Fecha Inicio:</label></td>
			<td><input type="date" name="fechain" value="<?php echo $modfei; ?>" required="required" placeholder='Fecha Inici&oacute;' title='Fecha Inici&oacute;'></td></tr>
            
			<tr><td><label>Fecha Final</label></td>
			<td><input type="date" name="fechafi" value="<?php echo $modfef; ?>" required="required" placeholder='Fecha Reparaci&oacute;n' title='Fecha Reparaci&oacute;n'></td></tr>
           
			 <tr><td><label>Comunidad</label></td>
			 <td><?php 
			 $b=new consultas();
			 echo $b->modselectdetabla("comunidad","nombre","idcomunidad",$modidco);
			     ?></td></tr>
				 
			 <tr><td><label>Obras en la Comunidad</label></td>
			 <td><?php 
			 $b=new consultas();
			 echo $b->modselectdetabla("obras","nombre","idobras",$modidob);
			     ?></td></tr>
						</table>
		...
		<?php }
			if (isset($_POST["codigo"]))
			{
				...
				$mov=new acciones();
				echo $mov->Modificar("riesgo","nombreri='$b',fechain='$c',fechafi='$d',idcomunidad='$e',idobras='$f'","idriesgo='$a'");
				}	?>
