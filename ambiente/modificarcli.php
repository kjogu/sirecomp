...
if (isset($_GET['id']))
	{
		...
		$id=$_GET['id'];
		$b=new consultas();
		$mod1=$b->buscarvalor("clima","clima","idclima='$id'");
		$modidme=$b->buscarvalor("clima","idmedioamb","idclima='$id'");
		$modidco=$b->buscarvalor("clima","idcomunidad","idclima='$id'");
 	...
 	if (isset($_GET['idb']))
	{
		...
		$id=$_GET['idb'];
		$b=new acciones();
		$mod1=$b->borrar("clima","idclima='$id'");
...
?>	
        <div id='infoprin'>  
		<form class="comunidad" method="POST" action="modificarcli.php" >
			<fieldset>
                <legend>
                Datos Clima
                </legend>
			...
			<table id="tabla">
				...
				<tr><td><label>Nombre de la Comunidad</label></td>
				 ...
				 <td><input type="text" name="clima" value="<?php echo $mod1; ?>" required="required" autofocus placeholder="Digite Clima" title="Digite Clima"></td></tr>
		...
		<?php }
			if (isset($_POST["codigo"]))
			{
				...
				$mov=new acciones();
				echo $mov->Modificar("clima","clima='$b',idmedioamb='$c',idcomunidad='$d'","idclima='$a'");  
		}	?>
