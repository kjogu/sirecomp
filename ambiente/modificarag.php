...
if (isset($_GET['id']))
	{
		...
		$id=$_GET['id'];
		$b=new consultas();
		$modre=$b->buscarvalor("recursoagua","idtiporeagua","idrecursoagua='$id'");
		$modar=$b->buscarvalor("recursoagua","area","idrecursoagua='$id'");
		$modzo=$b->buscarvalor("recursoagua","zona","idrecursoagua='$id'");
		$modidme=$b->buscarvalor("recursoagua","idmedioamb","idrecursoagua='$id'");
		$modidco=$b->buscarvalor("recursoagua","idcomunidad","idrecursoagua='$id'");
		...
		<div id="menu">
            <ul>
				...
				<li><a href='../index.php'>Cerrar Sesi&oacute;n</a></li>
						...
						<form action="modificarag.php" method="get">
						<table>
...
?>	
        <div id='infoprin'>  
		<form class="comunidad" method="POST" action="modificarag.php" >
			<fieldset>
                <legend>
                Datos de la Adesco
                </legend>
				...
				<table id="tabla">
					  ...
					  <tr><td><label title="Eliga Comunidad">Nombre Comunidad:</label></td>
						 <td><?php 
									$b=new consultas();
									echo $b->selectdetabla("comunidad","nombre","idcomunidad",$modidco);
									?></td></tr>
										  
					  <tr><td><label title="Eliga el Recurso">Tipo de Recurso:</label></td>
					  <td><?php 
									$b=new consultas();
									echo $b->selectdetabla("tiporeagua","recurso","idtiporeagua",$modre);
									?></td></tr>
					  
					  <tr><td><label>Area:</label></td>
					  <td><input type="text" name="area" id="area" value="<?php echo $modar; ?>" required="required" title="En metros Cuadrados"></td></tr>
                      
					  <tr><td><label>Zona Geograf&iacute;ca: </label></td>
					  <td><input type="text" name="zonageo" value="<?php echo $modzo; ?>" required="required" title="Seg&uacute;n Puntos Cardinales"></td></tr>
		...
		<?php
			...
			if (isset($_POST['codigo']))
			{
				...
				$mov=new acciones();
				echo $mov->Modificar("recursoagua","idtiporeagua='$b',area='$c',zona='$d',idmedioamb='$e',idcomunidad='$f'","idrecursoagua='$a'");
			}
			?>	
