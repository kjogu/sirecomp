...
if (isset($_GET['id']))
	{
		...
		$id=$_GET['id'];
		$b=new consultas();
		$mod1=$b->buscarvalor("comunal","nombre","idcomunal='$id'");
		$modar=$b->buscarvalor("comunal","area","idcomunal='$id'");
		$modfu=$b->buscarvalor("comunal","fundacion","idcomunal='$id'");
		$modidte=$b->buscarvalor("comunal","idterreno","idcomunal='$id'");
		$modidob=$b->buscarvalor("comunal","idcomunidad","idcomunal='$id'");
		$modidco=$b->buscarvalor("comunal","idcondicion","idcomunal='$id'");
 	...
 	if (isset($_GET['idb']))
	{
		...
		$id=$_GET['idb'];
		$b=new acciones();
		$mod1=$b->borrar("comunal","idcomunal='$id'");
		...
		<div id="menu">
            <ul>
				...
				<li><a href='../index.php'>Cerrar Sesi&oacute;n</a></li>
						...
						<form action="modificarco.php" method="get">
						<table>
...
?>	
        <div id='infoprin'>  
		<form class="comunidad" method="POST" action="modificarco.php" >
			<fieldset>
                <legend>
                Datos de la Comunal
                </legend>
			...
			<td><input type="text" name="nombre" value="<?php echo $mod1; ?>" required="required" autofocus placeholder='Digite Nombre Completo' title='Digite Nombre Completo'></td></tr>
            
			<tr><td><label>Fecha de Fundaci&otilde;n:</label></td>
			<td><input type="date" name="fechafu" value="<?php echo $modfu; ?>" required title='Seleccione Fecha Ignauraci&oacute;n'></td></tr>
            
			<tr><td><label>&Aacute;rea</label></td>
			<td><input type="text" name="area" value="<?php echo $modar; ?>" required="required" placeholder='Digite Metros Cuadrado' title='Digite Metros Cuadrado'></td></tr>
		...
		<?php }
			if (isset($_POST["codigo"]))
			{
				...
				$mov=new acciones();
				echo $mov->Modificar("comunal","nombre='$b',area='$c',fundacion='$d',idterreno='$e',idcomunidad='$f',idcondicion='$g'","idcomunal='$a'");
				}	?>
